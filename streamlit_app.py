...
def calculate_metrics():
    try:
        ...
        # FIX: Proper inventory value calculation
        ...
        pending_customers = len(customers[customers["pending_amount"] > 0])
        total_pending = customers["pending_amount"].sum()
        
        # Cash flow - FIX: Handle empty dataframes
...
if page == "dashboard":
    ...
    with col1:
        ...
        fig_profit = go.Figure(data=[go.Pie(
            labels=["Profit", "Loss"],
            values=[max(1, metrics["net_profit"]), max(1, metrics["total_revenue"]//2)],
            hole=0.4,
...
# ============================================================================
    ...
    try:
        ...
        customers_copy["value"] = customers_copy["digital_gold"] * metrics["gold_rate"]
        ...
        st.warning(f"Could not generate value chart: {str(e)}")
